...
func (d *volumeDriver) addVolume(name string, vol *nfsVolume) error {
	_, ok := d.volumes[name]
	if ok {
...
func (d *volumeDriver) getVolume(name string) (*nfsVolume, error) {
	v, ok := d.volumes[name]
	if !ok {
		return &nfsVolume{}, fmt.Errorf("volume %s not found", name)
	}
